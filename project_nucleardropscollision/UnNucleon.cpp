...
void InicieAnimacion(void){
  ...
  cout<<"unset key"<<endl;
  cout<<"set grid"<<endl;
  cout<<"set xrange [0:10]"<<endl;
  cout<<"set yrange [0:10]"<<endl;
  //cout<<"set xrange [40:60]"<<endl;
  //cout<<"set yrange [40:60]"<<endl;
...
int main(void){
  Cuerpo Nucleon[N+4]; int i;
  Colisionador Hertz;
  double t,Deltat=1e-4;
  ...
  //Nucleons
  //---------------(x0    ,y0  ,z0,Vx0            ,Vy0            ,Vz0,m0,R0);
  //  Nucleon[0].Inicie(Lx/5  ,Ly/2,0 ,VEL0*cos(Alpha),VEL0*sin(Alpha),  0,m0,R0);
  //  Nucleon[1].Inicie(4*Lx/5,Ly/2,0 ,-VEL0,0  ,  0,m0,R0);
  Nucleon[0].Inicie(4.9,Ly/2,0 ,0,1.0,0,m0,R0);
  Nucleon[1].Inicie(6.0,Ly/2,0 ,0.1,0,0,m0,R0);
