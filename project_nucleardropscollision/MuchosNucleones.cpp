...
const int Nx=5, Ny=5, Nz=2; const int N=Nx*Ny;
...
int main(void){
  Cuerpo Nucleon[N+4]; int i,ix,iy,iz;
  Colisionador Hertz;
  Crandom ran2(0);
  double t,Deltat=1e-2;
  double tmax,tdibujo; int ndibujos=10000;
  double m0=mAr,R0=3;
  double T,VEL0=10,Alpha,dx,dy,dz;
  VEL0=sqrt(2*Epp/mAr);
  T=Lx/VEL0;
  ...
  for(t=0,tdibujo=0;t<tmax;t+=Deltat,tdibujo+=Deltat){
    cout<<Nucleon[0].Getx()<<" "<<Nucleon[0].Gety()<<" "<<Nucleon[17].GetFx()<<endl;
