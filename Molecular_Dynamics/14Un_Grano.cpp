...
void Colisionador::CalculeFuerzaEntre(Cuerpo & Grano1, Cuerpo & Grano2){
  vector3D n , F2 , r21 = Grano2.r -Grano1.r;
  double d21 = norma(r21);
  double h= Grano2.R+Grano1.R-d21;
  double Fn;
  if(h>0){
...
int main(void){
  ...
  double T , VEL0=10, Alpha=M_PI/4;
    T=Lx/VEL0;
  double tmax = 5.0*T , tdibujo; int ndibujos=1000;
  double m0=1, R0=3;
  ...
  Grano[N+1].Inicie(Lx+10000 , Ly/2 ,0 , VEL0*cos(Alpha), 0 ,  0 , 1  , 10000 );
  Grano[N+2].Inicie(Lx/2 , -10000 ,0 , VEL0*cos(Alpha), 0 ,  0 , 1  , 10000);
  Grano[N+3].Inicie(Lx/2 , Ly+10000 ,0 , VEL0*cos(Alpha), 0,  0 , 1  , 10000);
