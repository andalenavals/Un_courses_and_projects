...
private:
public:
  void Estabilizar(Cuerpo * Nucleon, int N, vector3D r, int seed);
  void DeleVelocidad(Cuerpo * Nucleon, int N, vector3D V0);
  friend class Cuerpo;
  friend class Cluster;
  friend class Colisionador;
};
void Nucleo::Estabilizar(Cuerpo * Nucleon, int N, vector3D r, int  seed){
  int i,ix,iy,iz,j;
  Colisionador Hertz;
  Crandom ran2(seed);
...
//----------------------------------------- MAIN ----------------------------------------
//---------------------------------------------------------------------------------------
int main(int argc, char** argv ){//Energia , Semilla dos parametros de entrada

  string En, Sm, name1="NoClusters", name2="NoMonomeros", name3="NoClusterMayor", name4="EnergiasMonomeros";
  En = std::string(argv[1]); Sm= std::string(argv[2]);
  name1.append(En); name1.append("_");  name1.append(Sm);  name1.append(".dat");
  name2.append(En); name2.append("_");  name2.append(Sm);  name2.append(".dat");
  name3.append(En); name3.append("_");  name3.append(Sm);  name3.append(".dat");
  name4.append(En); name4.append("_");  name4.append(Sm);  name4.append(".dat");
  Imprimir Archivo;
  Archivo.Inicie(name1.c_str() , name2.c_str() , name3.c_str() , name4.c_str() );
  ...
  NUCLEO.Estabilizar( Nucleon1 ,N1,r1,seed);   NUCLEO.Estabilizar( Nucleon2,N2,r2,seed);
