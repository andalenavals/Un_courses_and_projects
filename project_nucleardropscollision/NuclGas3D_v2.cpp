...
void InicieAnimacion(void){
  ...
  cout<<"set grid"<<endl;
  cout<<"set xrange [-16:16]"<<endl;
  cout<<"set yrange [-16:16]"<<endl;
...
int main(void){
  ...
  double tmax,tdibujo; int ndibujos=50000;
  ...
  double xinf=-16 , xsup=16 , yinf= -16, ysup = 16, zinf=-16, zsup=16;
  ...
  /*
  for(i=0;i<N;i++)
    cout<<i<<" "<<Nucleon[i].Getx()<<" "<<Nucleon[i].Gety()<<" "<<Nucleon[i].Getz()<<endl;
  */
  for(t=0,tdibujo=0;t<tmax;t+=Deltat,tdibujo+=Deltat){
	...
	razon=sqrt(Epp/EPPP);
	for(i=0;i<N;i++) Nucleon[i].Inicie(Nucleon[i].Getx(),Nucleon[i].Gety(),Nucleon[i].Getz(),Nucleon[i].GetVx()*razon,Nucleon[i].GetVy()*razon,Nucleon[i].GetVz()*razon,m0,R0);
      }
    }
    //cout<<t<<" "<<Nucleon[N].Getx()<<" "<<Nucleon[N+1].Getx()<<" "<<Nucleon[N+2].Gety()<<" "<<Nucleon[N+3].Gety()<<" "<<Nucleon[N+4].Getz()<<" "<<Nucleon[N+5].Getz()<<endl;
